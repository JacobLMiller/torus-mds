import { Descent, PseudoRandom } from './descent.ts';
import { LayoutHelper } from './layouthelper.ts';

function getGraphDistance(graph: any, sourceIndex: number, targetIndex: number): number {
    const numericDistance = graph.shortestPathLengths?.[sourceIndex]?.[targetIndex];
    if (Number.isFinite(numericDistance)) {
        return numericDistance;
    }

    const path = graph.shortestPath?.[sourceIndex]?.[targetIndex];
                ...
                for (let i = 0; i < this.randomOrder.nodeIndex.length; i++) {
                    ...
                    for (let j = 0; j < this.randomOrderArray.length; j++) {
                        ...
                        if (!this.visitedMatrix[a][b]) {
                            ...
                            let idealDistance =
                            this.configuration.linkLength * getGraphDistance(this.graph, a, b);
