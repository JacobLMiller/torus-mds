...
graphs (GRG) -- euclidean and toroidal variants. Persists layouts + a run
manifest under --output-dir; does not compute metrics (see compute_metrics.py
for that, run as a separate phase).

Usage:
    python grg_comparison.py [--n-graphs N] [--seed S] [--output-dir layouts/grg]
    python grg_comparison.py --graph-type-weights 0.6,0.4
"""
...
def _connect_and_stitch(
    ...
    """Shared connectivity-retry/stitch loop for both GRG generators."""
...
def grg_cached_graph_iterator(cache_dir: str, shard_index: int, num_shards: int):
    """Read graphs pregenerated by stage_grg.py instead of generating them here."""
    ...
    for _, row in manifest.iterrows():
        ...
        yield GraphRecord(
            exp_idx=exp_idx,
            graph=G,
            meta={"epsilon": float(row.epsilon), "eps_actual": float(row.eps_actual),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-graphs", type=int, default=1000,
                        help="Number of GRG graphs to generate (default: 1000; ignored with --cache-dir)")
    ...
    parser.add_argument("--eps-min", type=float, default=0.10,
                        help="Minimum connection radius epsilon (default: 0.10; ignored with --cache-dir)")
    parser.add_argument("--eps-max", type=float, default=0.40,
                        help="Maximum connection radius epsilon (default: 0.40; ignored with --cache-dir)")
    parser.add_argument("--n-eps", type=int, default=6,
                        help="Number of discrete epsilon values in grid (default: 6; ignored with --cache-dir)")
    parser.add_argument("--graph-type-weights", type=str, default="1,1",
                        help="Comma-separated euclidean,toroidal sampling weights "
                             "(default: 1,1 = equal halves; ignored with --cache-dir)")
    parser.add_argument("--cache-dir", type=str, default=None,
                        help="Read graphs pregenerated by stage_grg.py from this directory instead of "
    ...
    if args.cache_dir:
        ...
        graph_iterator = grg_cached_graph_iterator(
