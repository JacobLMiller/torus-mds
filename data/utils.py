# This file is almost verbatim from Kohli et al. 2024 RATS: Unsupervised manifold learning using low-distortion
# alignment of tangent spaces
...
def get_path():
    ...
    path = Path((HERE / "my_path").read_text().strip())
    return path

data_dir = os.path.join(get_path(), 'grid_cells/Toroidal_topology_grid_cell_data/')
...
def pca(data, dim=2):
    ...
    return components, var_exp, evals[:dim], evecs
...
def sample_denoising_(data, k = 10, metric = 'euclidean'):    
    n = data.shape[0]
...
def get_spikes(rat_name, mod_name, day_name = '', sess_name = '', bType = None, bSmooth = True, 
    ...
    if str.upper(rat_name) == 'R':
        f = np.load(data_dir + '/rat_r_' + day_name + '_grid_modules_1_2_3.npz', allow_pickle = True)
    elif str.upper(rat_name) == 'Q':
        f = np.load(data_dir + '/rat_q_grid_modules_1_2.npz', allow_pickle = True)
    elif str.upper(rat_name) == 'S':
        f = np.load(data_dir + '/rat_s_grid_modules_1.npz', allow_pickle = True)
    else:
    ...
    if bType:    
        ...
        if len(day_name)>0:
            is_conj = f['is_conj_' + str.upper(rat_name) + mod_name + '_' + day_name]
        else:
            is_conj = f['is_conj_' + str.upper(rat_name) + mod_name + day_name]
        f.close()
        if bType == 'conj':
    ...
    if gridclass and False: 
        f = np.load(data_dir + 'Results/grid_cell_classes_indices.npz', allow_pickle = True)
        ind = f[str.upper(rat_name) + mod_name + '_ind']
        f.close()
    ...
    if str.upper(sess_name[:3]) in ('SWS', 'REM'):
        times = times_all['rat_' + str.lower(rat_name) + '_sleep' + day_name]
    elif str.upper(sess_name[:3]) == 'WW2':
        times = times_all['rat_' + str.lower(rat_name) + '_' + sess_name + day_name]        
    else:
        times = times_all['rat_' + str.lower(rat_name) + '_' + sess_name[:2] + day_name]        
    ...
    if not bBinned:
        ...
        if (rat_name =='S') & (sess_name == 'WW'):
            times2 = times_all['rat_' + str.lower(rat_name) + '_' + sess_name + '2' + day_name]        
        ...
        if sess_name[:2] in ('OF', 'WW'):
            ...
            start = np.array(times_all['rat_' + str.lower(rat_name) + '_' + sess_name[:3] + day_name])[:,0]
            start = start[start<max_time0]

            end = np.array(times_all['rat_' + str.lower(rat_name) + '_' + sess_name[:3] + day_name])[:,1]
        ...
        if bSmooth:
            ...
            ker = np.exp(-np.power(np.arange(thresh+1)/res, 2)*sig2) # Dhruv TODO: Replace with Boris & Xiu's kernel
        ...
        if sess_name[:2] in ('OF', 'WW'):
            ...
            start = np.array(times_all['rat_' + str.lower(rat_name) + '_' + sess_name[:3] + day_name])[:,0]*res
            start = start[start<max_time]

            end = np.array(times_all['rat_' + str.lower(rat_name) + '_' + sess_name[:3] + day_name])[:,1]*res
...
def load_pos(rat_name, sess_name, day_name = '', bSpeed = False):    
    ...
    if str.upper(sess_name) in ('SWS', 'REM'):
        times = times_all['rat_' + str.lower(rat_name) + '_sleep' + day_name]
    else:
        times = times_all['rat_' + str.lower(rat_name) + '_' + sess_name + day_name]        
    ...
    if (str.upper(rat_name) == 'R') & (day_name == 'day1'):
        if sess_name =='OF':
            valid_times = np.concatenate((np.arange(0, (14778-7457)*res/dt),
                                          np.arange((14890-7457)*res/dt, (16045-7457)*res/dt))).astype(int)
        elif sess_name == 'WW':
    ...
    if bSpeed:
        ...
        speed = np.concatenate(([speed[0]],speed))
        if (str.upper(rat_name) =='S') & (sess_name == 'WW'):
...
def plot_barcode(persistence, file_name = '', xmax=None):
...
def toroidal_alignment(rat_name, mod_name, sess_name_0, sess_name_1, day_name, bPlot = False):
    ...
    ############################## Load 1 ############################
    f = np.load(data_dir + 'Results/' + file_name_1  + '_decoding.npz',
        allow_pickle = True)
    call = f['coords']
    ...
    f.close()    
    ############################## Load 2 ############################
    f = np.load(data_dir + 'Results/' + file_name_2 + '_decoding.npz',
        allow_pickle = True)
    call = f['coords']
    ...
    ############################## compare ############################
    f = np.load(data_dir + 'Results/' + file_name_1 + '_para.npz', allow_pickle = True)
    p1b_1 = f['p1b_1']
    p2b_1 = f['p2b_1']
    m1b_1 = f['m1b_1']
    m2b_1 = f['m2b_1']
    ...
    f = np.load(data_dir + 'Results/' + file_name_2 + '_para.npz', allow_pickle = True)
    p1b_2 = f['p1b_1']
    p2b_2 = f['p2b_1']
    m1b_2 = f['m1b_1']
    m2b_2 = f['m2b_1']
    ...
    np.savez_compressed(data_dir + 'Results/' + file_name + '_alignment_dec',
                        pshift = pshift,
                        cbox = cbox1,
...
def fit_para(rat_name, mod_name, sess_name, day_name):
    ...
    if len(day_name)>0:
        file_name += '_' + day_name  
    ############################## Load 1 ############################
    f = np.load(data_dir + 'Results/' + file_name  + '_decoding.npz',
        allow_pickle = True)
    ...
    np.savez_compressed(data_dir + 'Results/' + file_name + '_para', 
        p1b_1 = p1b_1, 
        p2b_1 = p2b_1,
        m1b_1 = m1b_1, 
        m2b_1 = m2b_1, 
